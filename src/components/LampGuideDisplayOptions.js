...
const LampGuideGrid = () => {
  ...
  const { productColumns } = useSelector((state) => state.products);

  const [availableColumns, setAvailableColumns] = useState(['Ballast Text', 'Blank Column', 'Case Price', 'Extended Price', 'Fixture Description', 'Life Expectancy (Months)', 'Location In Store', 'Max Order Qty', 'Price']);
  ...
  useEffect(() => {
    const fetchColumns = async () => {
      try {
        dispatch(fetchProductColumns({ userid, accountid, storeid }));
        setColumns([]);
      } catch (error) {
  ...
  useEffect(() => {
    if (productColumns && productColumns.length > 0) {
      const apiColumns = productColumns.map((col) => ({
        accountLabel: col.accountColumnName || '',
        width: col.accountColumnWidth || '',
        orderId: col.orderID || '',
        hideOnPDF: col.doNotDisplayDataOnPDF || false,
        textColor: col.colorHexCode || '#000000',
        alignment: col.cellAlignment === 'C' ? 'center' : col.cellAlignment === 'L' ? 'left' : 'right'
      }));

      setColumns(apiColumns);
      const usedColumnNames = new Set(apiColumns.map((col) => col.accountLabel));
      setAvailableColumns((prev) => prev.filter((col) => !usedColumnNames.has(col)));
    }
  }, [productColumns]);
        ...
        <div className='flex flex-col lg:flex-row gap-8'>
          ...
          <div className='lg:w-1/3'>
            ...
            <ul className='border border-[#4B449D] rounded-lg bg-white shadow-md p-4 max-h-80 overflow-y-auto'>
              {availableColumns.map((column, index) => (
                <li key={index} className='flex justify-between items-center py-2 px-3 mb-2 bg-[#F6F5FF] rounded-lg hover:bg-[#E4E2FF]'>
          ...
          <div className='lg:w-2/3'>
                ...
                <tbody>
                  {columns.map((column, index) => (
                    <tr key={index} className='hover:bg-[#F6F5FF]'>
                      <td className='border px-4 py-2'>{column.accountLabel || 'Custom Column'}</td>
                      <td className='border px-4 py-2'>
                        <input type='text' className='w-full border rounded px-2 py-1' placeholder='Enter Account Label' value={column.accountLabel} onChange={(e) => updateColumn(index, 'accountLabel', e.target.value)} />
                      </td>
                      <td className='border px-4 py-2'>
                        <input type='number' className='w-full border rounded px-2 py-1' placeholder='Width (%)' min='0' max='100' value={column.width} onChange={(e) => updateColumn(index, 'width', e.target.value)} />
                      </td>
                      <td className='border px-4 py-2'>
                        <input type='number' className='w-full border rounded px-2 py-1' placeholder='Order ID' min='1' value={column.orderId} onChange={(e) => updateColumn(index, 'orderId', e.target.value)} />
                      </td>
                      <td className='border px-4 py-2 text-center'>
                        <input type='checkbox' checked={column.hideOnPDF} onChange={(e) => updateColumn(index, 'hideOnPDF', e.target.checked)} />
                      </td>
                      <td className='border px-4 py-2'>
                        <input type='color' className='w-full border rounded' value={column.textColor} onChange={(e) => updateColumn(index, 'textColor', e.target.value)} />
                      </td>
                      <td className='border px-4 py-2'>
                        <select className='w-full border rounded px-2 py-1' value={column.alignment} onChange={(e) => updateColumn(index, 'alignment', e.target.value)}>
                      ...
                      <td className='border px-4 py-2 text-center'>
                        <button className='text-red-500 hover:underline' onClick={() => removeColumn(index)}>
                          Remove
                        </button>
                      </td>
                    </tr>
            ...
            <div className='flex justify-end mt-4'>
              <button className='btn bg-[#4B449D] text-white px-6 py-2 rounded hover:bg-[#3C387C]' onClick={addColumnManually}>
                Add New Column
              </button>
              ...
              <div className='flex flex-col gap-4'>
                <label className='flex items-center'>
                  <input
                    ...
                    onChange={(e) => updateSortingOptions('sortByVendorPartNumber', e.target.checked)}
                    style={{
                ...
                <label className='flex items-center'>
                  <input
                    ...
                    onChange={(e) => updateSortingOptions('sortByProductCategory', e.target.checked)}
                    style={{
                ...
                <label className='flex items-center'>
                  <input
                    ...
                    onChange={(e) => updateSortingOptions('sortByProductSubCategory', e.target.checked)}
                    style={{
                ...
                <label className='flex items-center'>
                  <input
                    ...
                    onChange={(e) => updateSortingOptions('sortBySequenceNumber', e.target.checked)}
                    style={{
              ...
              <div className='flex flex-col gap-4'>
                <label className='flex items-center'>
                  <input
                    ...
                    onChange={(e) => updateSortingOptions('createPrintedHeaders', e.target.checked)}
                    style={{
                ...
                <label className='flex items-center'>
                  <input
                    ...
                    onChange={(e) => updateSortingOptions('createOnScreenHeaders', e.target.checked)}
                    style={{
